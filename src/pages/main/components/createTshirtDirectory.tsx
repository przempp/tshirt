...
function CreateTshirtDirectoryNew() {
  ...
  let tshirtDirectory: (JSX.Element | JSX.Element[])[];
  tshirtDirectory = [];
  data.collections.items.forEach(
    (
      collections: { productVariants: { items: any[] }; name: string },
      i: number
    ) => {
      collections.productVariants.items.forEach((products: any, i: number) => {
        ...
        if (i === 7 && collections.name === "Original") {
          tshirtDirectory.push(
            <div className="directory-sticker menu-item col-md-6 col-lg-4 align-self-center">
              <div className="col-sm ">
                <img
                  alt="product"
                  className="directory-tshirt tshirt-shadow animation"
                  src={selectPrint}
                />
              </div>
            </div>
      ...
      <div className="menu-item  col-md-6 col-lg-4">
        <div style={{ minHeight: `250px` }} className="col-sm tshirt-div">
          <Link to={"/tshirts/" + item.product.slug}>
            <img
              alt="product"
              className={`directory-tshirt tshirt-shadow`}
              src={`${item.featuredAsset.preview}?preset=large&format=webp`}
            />
          </Link>
        </div>
        <div className="col-sm menu-tshirt-desc">
          <p className="menu-tshirt-desc-name">
            <Link to={"/tshirts/" + item.product.slug}>
              {" "}
              {item.product.name}
            </Link>
