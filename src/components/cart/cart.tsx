...
function Cart() {
  ...
  return (
    <OutsideClickHandler onOutsideClick={() => setSidebarClassname("")}>
      <div
        style={{ zIndex: "100022" }}
        className={` position-fixed  cart-icon `}
      >
        <h3
          onClick={() => {
            sidebarClassname
              ? setSidebarClassname("")
              : setSidebarClassname("enter");
          }}
          className="pt-2 pl-1"
        >
          {activeOrderData && activeOrderData.activeOrder
            ? activeOrderData.activeOrder.totalWithTax / 100
            : "0"}
          $
        </h3>
      ...
      <div className={`sidebar  ${sidebarClassname}`}>
        ...
        {activeOrderData &&
          ...
          activeOrderData.activeOrder.totalQuantity !== 0 && (
